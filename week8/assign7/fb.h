 ...
 */
void fb_init(unsigned int width,
  unsigned int height,
  unsigned int depth_in_bytes,
  fb_mode_t mode);

/*
 * Modify y_offset of the underlying framebuffer
 */
void fb_set_y_offset(int y_offset);
 ...
 * the depth in bytes per pixel. However, the pitch may be greater than that if
 * the GPU elects to add padding to the end of the row.
 *
